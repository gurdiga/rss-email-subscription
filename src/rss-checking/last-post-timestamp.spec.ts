...
describe('Last post timestamp', () => {
  ...
  describe(recordLastPostTimestamp.name, () => {
    it('writes pubDate of the latest item to data/lastPostTimestamp.json', () => {
      ...
      const initialRssItems = [...rssItems];

      recordLastPostTimestamp(mockDataDir, rssItems, mockWriteFile);

      const expectedFileContent = JSON.stringify({
        lastPostTimestamp: rssItems[2].pubDate,
      });

      expect(rssItems).to.deep.equal(initialRssItems, 'Does not alter the input array');
