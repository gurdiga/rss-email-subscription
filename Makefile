...
lint-mocha-only:
	@# BASH_ENV sources ~/.bash_profile which sets pipefail; on macOS,
	# xargs exits 123 when grep finds no match, and pipefail would propagate that
	set +o pipefail
...
# within the window. Only this target sets it — prod deploys with `make app start`,
# and an unset value means rate limiting is on.
